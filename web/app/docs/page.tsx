...
export default function DocsPage() {
  ...
  useEffect(() => {
    ...
    script.onload = () => {
      ...
      (window as any).mermaid.run({ nodes: containerRef.current?.querySelectorAll(".mermaid") });
      setLoaded(true);
    };
  ...
  const diagrams = [
    ...
    actor User
    participant HUD
    participant Clipboard as Win32 Clipboard
    participant Target as Target IDE/Editor

    User->>HUD: Press F9 (or ..p)
    HUD->>HUD: Extract code from output
    alt No code ready
        HUD-->>User: Show "Press F5 first"
    else Code ready
        HUD-->>User: "Pasting in 2s — click target"
        HUD-->>User: "Pasting in 1s — click target"
        HUD->>Clipboard: OpenClipboard + SetClipboardText
        HUD->>Target: Simulate Ctrl+V
        Target-->>User: ✅ Full code pasted instantly
        HUD-->>User: "Pasted N chars ✓"
    end`,
    },
    {
    ...
    else Wrong machine / expired
        API-->>User: {valid:false}
    end
    User->>User: Fernet cache → spawn ctfmon.exe`,
    },
    {
  ...
  return (
    <div style={{ background: "#0f1117", minHeight: "100vh", padding: "40px 24px", fontFamily: "'Segoe UI', system-ui, sans-serif" }}>
      <div style={{ maxWidth: 900, margin: "0 auto" }}>
        <div style={{ textAlign: "center", marginBottom: 48 }}>
          <div style={{
            display: "inline-flex", alignItems: "center", gap: 12,
            background: "rgba(168,180,255,0.08)", border: "1px solid rgba(168,180,255,0.2)",
            borderRadius: 12, padding: "8px 20px", marginBottom: 20
          }}>
            <span style={{ fontSize: 20 }}>⚡</span>
            <span style={{ color: "#a8b4ff", fontSize: 12, fontWeight: 700, letterSpacing: "0.2em", textTransform: "uppercase" }}>TITAN v2.0</span>
          </div>
          <h1 style={{ color: "#e2e8f0", fontSize: 32, fontWeight: 800, marginBottom: 8 }}>Ecosystem Visual Workflow</h1>
          <p style={{ color: "#6b7280", fontSize: 14 }}>Multi-Model AI · Hardware-Locked Licensing · Full Flow Diagrams</p>
        </div>
        ...
        {diagrams.map((d, i) => (
          <div key={i} style={{
            ...
            <h2 style={{ color: "#a8b4ff", fontSize: 13, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.15em", marginBottom: 20, paddingBottom: 12, borderBottom: "1px solid #2a2d3e" }}>
              {d.title}
            </h2>
            <div className="mermaid" style={{ display: "flex", justifyContent: "center", minHeight: 80 }}>
              {d.code}
            </div>
          </div>
        ))}

        <div style={{ background: "#1a1d27", border: "1px solid #2a2d3e", borderRadius: 16, padding: 28, marginBottom: 28 }}>
          <h2 style={{ color: "#a8b4ff", fontSize: 13, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.15em", marginBottom: 20, paddingBottom: 12, borderBottom: "1px solid #2a2d3e" }}>
            ...
            <thead>
              <tr style={{ background: "#252836" }}>
                {["Model", "MCQ", "Code", "Key Format", "Status"].map(h => (
                  <th key={h} style={{ padding: "10px 14px", color: "#6b7280", fontSize: 11, textTransform: "uppercase", letterSpacing: "0.1em", textAlign: "left" }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              ...
              ].map(([model, mcq, code, key, status, color], ri) => (
                <tr key={ri} style={{ borderTop: "1px solid #2a2d3e" }}>
                  <td style={{ padding: "10px 14px", color: "#e2e8f0" }}>{model}</td>
                  <td style={{ padding: "10px 14px", color: "#e2e8f0" }}>{mcq}</td>
                  <td style={{ padding: "10px 14px", color: "#e2e8f0" }}>{code}</td>
        ...
        <p style={{ textAlign: "center", color: "#374151", fontSize: 12, marginTop: 32 }}>
          TITAN Ecosystem Docs · v2.0 · Generated 2026-05-12
        </p>
      </div>
    </div>
  );
